...
server.serveStatic("/", SD, "/www/").setDefaultFile("index.html");      //hosting files from sd card stored in www folder
  ...
  server.on("/do/start", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    if( STOP_CONDITION == 1)         //to check if the recording is started without transferring
    {
      START_CONDITION = 0;
      STOP_CONDITION = 0;
      digitalWrite(RED, LOW);
      
      request->send(200, "text/plain", "Starting recording without transfer");
    }
     ...
     else
    {
      ...
      if(START_CONDITION == 0)        //to check if we have multiple start requests
      {
